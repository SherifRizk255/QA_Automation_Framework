...
export class AssetPortalLoginPage {
  ...
  async goto(): Promise<void> {
    await allure.step('Navigate to the IScore Asset Portal login page', async () => {
      await this.page.goto(ROUTES.assetPortal.login);
  ...
  async loginWithConfiguredUser(): Promise<void> {
    await allure.step('Sign in with the configured IScore Asset Portal user', async () => {
      await this.usernameInput().fill(ENV.assetPortal.username);
      await this.passwordInput().fill(ENV.assetPortal.password);
  ...
  async assertLoginFormRendered(): Promise<void> {
    await allure.step('Assert the login form is rendered', async () => {
      await expect(this.repository.locator('ASSET.LOGIN.FORM_ROOT')).toBeVisible();
  ...
  private usernameInput(): Locator {
    return this.repository.locator('ASSET.LOGIN.USERNAME_INPUT');
  }

  private passwordInput(): Locator {
    return this.repository.locator('ASSET.LOGIN.PASSWORD_INPUT');
  }

  private submitButton(): Locator {
    return this.repository.locator('ASSET.LOGIN.SUBMIT_BUTTON');
  }
}
