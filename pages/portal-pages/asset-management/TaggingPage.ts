...
import { AddTrackingDialogComponent } from '../../components/portal/asset-management/AddTrackingDialogComponent';
import { AdvancedFilterComponent } from '../../components/portal/asset-management/AdvancedFilterComponent';
import { AssetSelectionGridComponent } from '../../components/portal/asset-management/AssetSelectionGridComponent';
import { PortalHeaderComponent } from '../../components/portal/asset-management/PortalHeaderComponent';
import { AssetPortalBasePage } from './AssetPortalBasePage';

const TAGGING_MODULE_LABEL = 'Tagging';

export class TaggingPage extends AssetPortalBasePage {
  ...
  async openByRoute(): Promise<void> {
    await allure.step('Navigate directly to the Tagging route', async () => {
      await this.page.goto(portalHashRoute(ROUTES.assetPortal.tagging, ENV.assetPortal.loginUrl));
      await this.assertTaggingModuleLoaded();
    });
  }
  ...
  async assertTaggingModuleLoaded(): Promise<void> {
    await allure.step('Assert the Tagging module is loaded', async () => {
      await expect(this.repository.locator('ASSET.HEADER.ACTIVE_MODULE_LABEL')).toHaveText(
  ...
  private addTrackingButton(): Locator {
    return this.repository.locator('ASSET.TAGGING.ADD_TRACKING_BUTTON');
  }

  private submitButton(): Locator {
    return this.repository.locator('ASSET.TAGGING.SUBMIT_BUTTON');
  }
}
