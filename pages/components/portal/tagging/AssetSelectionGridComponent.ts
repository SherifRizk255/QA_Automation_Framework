 ...
 * `role=cell`. Filtering by TAGGING.ROW_CELL excludes the header
...
export class AssetSelectionGridComponent {
  ...
  async readRowIdentifier(index: number): Promise<string> {
    const identifierCell = this.repository.locator('TAGGING.ASSET_IDENTIFIER_CELL', {
  ...
  private dataRows(): Locator {
    const allRows = this.repository.locator('TAGGING.ROW', { scope: this.gridRoot });
    return allRows.filter({ has: this.repository.locator('TAGGING.ROW_CELL') });
  }
  ...
  private selectAllCheckbox(): Locator {
    return this.repository.locator('TAGGING.SELECT_ALL_CHECKBOX', { scope: this.gridRoot });
  }

  private emptyState(): Locator {
    return this.repository.locator('TAGGING.EMPTY_STATE');
  }
}
