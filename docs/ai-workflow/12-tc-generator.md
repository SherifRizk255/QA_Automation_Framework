...
| Preconditions | Transfer role exists. Test account available. |
| TC Setup | Login User. Create Beneficiary. Verify Account Balance. |
| Steps | Numbered, specific, action-oriented. What the tester does. |
| TC Teardown | Delete Beneficiary. Logout User. |
...
Contains lifecycle actions required before execution.
...
Contains lifecycle actions required after execution.
...
Consume lifecycle planning generated by Traceability Manager.

Lifecycle planning is authoritative.

TC Generator must not invent lifecycle actions.
...
## Lifecycle Generation Rules
 1. Authentication Dependency
  ...
  4. Shared Dependency
     ...
     - SUITE_SETUP Candidate
     - SUITE_TEARDOWN Candidate
...
## LIFECYCLE RETRY AWARENESS

Lifecycle actions may be marked:
...
- Every generated TC must include lifecycle evaluation.
- Lifecycle actions must originate from Lifecycle Matrix.
- Lifecycle actions must follow docs/test-design/test-lifecycle.md.
- Common lifecycle actions should be reusable.
- TC_SETUP and TC_TEARDOWN must retain traceability.
