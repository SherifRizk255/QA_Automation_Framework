...
test.describe('IScore Asset Management - locator repository integrity', () => {
  ...
  test('TC-FW-ASSET-011 | Locator repository entries were registered', () => {
    const { entries } = loadRepository();
    expect(entries.length).toBeGreaterThan(0);
  });

  test('TC-FW-ASSET-012 | Every entry has an owner, a screen name, and a resolvable primary locator type', () => {
    const { entries } = loadRepository();
  ...
  test('TC-FW-ASSET-013 | Every entry follows the SCREEN.ELEMENT naming convention', () => {
    const { entries } = loadRepository();

    for (const entry of entries) {
      expect(entry.elementId, entry.elementId).toMatch(/^[A-Z]+(\.[A-Z0-9_]+){1,}$/);
    }
  });
